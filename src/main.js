...
function setupScene(){
 ...
 const signatureCanvas=document.createElement('canvas');signatureCanvas.width=512;signatureCanvas.height=256;const ctx=signatureCanvas.getContext('2d');ctx.fillStyle='#31352c';ctx.font='italic 67px Georgia';ctx.translate(35,155);ctx.rotate(-.13);ctx.fillText('R. Mutt',0,0);ctx.font='45px Georgia';ctx.fillText('1917',50,57);const texture=new THREE.CanvasTexture(signatureCanvas);texture.colorSpace=THREE.SRGBColorSpace;const signature=add(new THREE.PlaneGeometry(.54,.27),new THREE.MeshBasicMaterial({map:texture,transparent:true,depthWrite:false,side:THREE.DoubleSide}));signature.position.set(.64,-.47,.322);signature.rotation.z=-.55;signature.rotation.y=.37;
